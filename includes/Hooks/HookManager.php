...
class HookManager {
	...
	private function init() {
		...
		add_filter( 'wp_insert_post_data', array( $this, 'translate_post_slug' ), 10, 2 );

		// Hook for media filename translation.
		add_filter( 'sanitize_file_name', array( $this, 'translate_media_filename' ), 10, 1 );

		// Hook for attachment post name (slug).
		add_filter( 'wp_insert_attachment_data', array( $this, 'translate_attachment_slug' ), 10, 2 );
	}
	...
	public function translate_post_slug( $data, $postarr ) {
		...
		// Skip if this is an update and slug is already manually set (not auto-generated).
		if ( ! empty( $postarr['ID'] ) && ! empty( $data['post_name'] ) ) {
			// Allow translation if slug is auto-draft or auto-generated from title.
			$auto_generated_slugs = array(
				'auto-draft',
				sanitize_title( $data['post_title'] ),
			);

			// If current slug is not auto-generated, skip translation.
			if ( ! in_array( $data['post_name'], $auto_generated_slugs, true ) ) {
				return $data;
			}
		}
		...
		if ( ! $this->needs_translation( $data['post_title'] ) ) {
			// If title doesn't need translation but slug is auto-draft, set proper slug.
			if ( 'auto-draft' === $data['post_name'] ) {
